...
?>';
		...
		$q->execute();
		$q = $conn->prepare("CREATE TABLE application ( id int(11) NOT NULL, draft_id varchar(20) NOT NULL, applicant int(11) NOT NULL, receiver int(11) NOT NULL, message text NOT NULL, application_date varchar(10) NOT NULL, department varchar(50) NOT NULL, req_date varchar(10) NOT NULL, ending_date varchar(10) NOT NULL, vehicle_req varchar(50) NOT NULL, vehicle_issue varchar(20) NOT NULL, notification varchar(1) NOT NULL, issued_by varchar(20) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
		$q->execute();
		...
		$q->execute();
		$q = $conn->prepare("CREATE TABLE vehicle ( num varchar(20) NOT NULL, type varchar(20) NOT NULL, subtype varchar(20) NOT NULL, status varchar(10) NOT NULL, location varchar(30) NOT NULL, app_id varchar(20) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
		$q->execute();
		$q = $conn->prepare("ALTER TABLE application ADD PRIMARY KEY (id), ADD UNIQUE KEY draft_id (draft_id);");
		$q->execute();
		$q = $conn->prepare("ALTER TABLE user ADD PRIMARY KEY (id), ADD UNIQUE KEY mail_id (mail_id);");
		$q->execute();
		$q = $conn->prepare("ALTER TABLE vehicle ADD PRIMARY KEY (num);");
		$q->execute();
		$q = $conn->prepare("ALTER TABLE application MODIFY id int(11) NOT NULL AUTO_INCREMENT;");
		$q->execute();
